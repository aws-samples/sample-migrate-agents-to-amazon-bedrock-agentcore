...
![Diagram titled "The same agent. Less of it is yours to operate." Subtitle: three alternatives, choose who owns the orchestration loop. At the top, the baseline you have today is self-hosted LangGraph, and a single unbroken bar labelled "one agent, purpose stays constant" states what the agent does: classify, route, escalate, three tools. Below it a panel headed "the ten things you operate today" lists VPC, WAF, IAM policies, secrets rotation, OS patching, dependency updates, auto-scaling rules, session isolation, checkpoint storage, and tool auth. An Amazon Bedrock AgentCore icon marks a split into three alternative paths rather than sequential steps. Replatform, stage 1 and shipped in this repository, keeps your loop and adopts AgentCore Runtime, Gateway, and Memory. Rebuild, stage 2 and also shipped, changes the loop to model-driven planning and adds a Cedar-based policy engine. Both strike through the same seven items in place and leave the same three unstruck, IAM policies, secrets rotation, and dependency updates, so each reports seven transferred and three remaining. The third path, handing the loop over to AgentCore, is marked not in this sample and prose only, carries no code-backed reduction claim, and leaves all ten items unstruck.](images/agentcore-migration-architecture.png)
